...
def get_indices(batch_size, sample_num, point_num, sampling, pool_setting=None):
...
def augment(points, xforms, range=None):
    ...
    all_ones = range * tf.ones(tf.shape(jitter_data),name='all_ones')
    jitter_clipped = tf.clip_by_value(jitter_data, -5 * range * all_ones, 5 * range * all_ones, name='jitter_clipped')
